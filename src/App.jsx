...
@media (prefers-reduced-motion: reduce){
  ...
  .pulse-on-change{animation:none;}
}
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              type="text"
              value={localAdminPw}
              onChange={(e) => setLocalAdminPw(e.target.value)}
              className="field w-full rounded-lg border border-stone-200 bg-white px-3 py-2 text-sm mt-1 text-stone-700"
            />
          </div>
          ...
          <div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr className="text-left text-stone-400 uppercase tracking-wide">
                  ...
                  {participants.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="border-t border-stone-100">
                        <td className="py-1 pr-2 text-stone-600">{p.name}</td>
                        ...
                        <td className="py-1 pr-2 text-center">
                          <input
                            ...
                            className="h-4 w-4 accent-[#1C3B2E]"
                          />
                        </td>
                      </tr>
                    );
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            ...
            <button
              ...
              style={{ color: BRAND.ember, border: `1px solid ${BRAND.ember}` }}
            >
              Reset to defaults
            </button>
          </div>
    ...
    <div className="space-y-5">
      ...
      <Card className="p-4">
        <div className="flex items-center justify-between text-xs text-stone-500">
          ...
          <span>{balance >= 0 ? "You're covered" : "You owe"}</span>
        </div>
        ...
        <div className="mt-2 text-xs text-stone-500">
          {balance >= 0
            ? `You have ${fmt(balance)} left over.`
            : `You need to pay ${fmt(Math.abs(balance))} more.`}
        </div>
      </Card>
      ...
      <Card className="p-4">
        <SectionHeading eyebrow="Quick look" title="Upcoming on your trip" />
        {upcoming.length === 0 ? (
      ...
      >
        ...
        <div className="w-full max-w-sm rounded-2xl bg-white p-8 shadow-lg">
          ...
          <p className="mt-4 text-center text-xs text-stone-400">
            Shared credentials — ask your trip organiser.
          </p>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <main className="mx-auto max-w-3xl px-4 py-4">
        ...
        {tab === "profile" && !isAdmin && role === "member" && (
          <MemberProfile
